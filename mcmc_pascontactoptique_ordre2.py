...
def model(theta, i):
    A, phi1, alpha, beta, gamma = theta
    model = A * ( np.cos(phi1) - np.cos( alpha + beta * i + gamma * i**2 - phi1))
    return model
...
def lnprior(theta):
    A, phi1, alpha , beta, gamma = theta
    if 21 < A < 25 and 0 <= phi1 <= 2*np.pi and -1 < alpha < 0  and  1 < beta < 4  and 0 < gamma < 5:
        return 0.0
    else:
        return -np.inf
...
# set niter
niter = 6000
initial = np.array([23,  0.2 , -0.5 , 2.95, 3.5])  # initial for A, phi1, alpha, beta and gamma
ndim = len(initial)
p0 = [np.array(initial) + [0.05, 0.05, 0.05, 0.05, 0.1] * np.random.randn(ndim) for i in range(nwalkers)]
...
gamma_samples = np.array(samples)[:,4]
...
plt.plot(i,best_fit_model,label='Fit Likelihood \nA = '+str(np.round(theta_max[0], 2))+r", $\phi_1$ ="+str(np.round(theta_max[1], 2))
                                ...
                                +r" et $\gamma$ = "+str(np.round(theta_max[4],2)),
                                color = 'salmon')
...
columns = [
    fits.Column(name='Parametre', format='20A', array=['A', 'phi1', 'alpha', 'beta', 'gamma','delta']),
